describe "using pact publications (migrate 31-32)", migration: true do
  ...
  let!(:consumer_1) { create(:applications, {name: "Consumer 1", created_at: now, updated_at: now}) }
  let!(:provider_1) { create(:applications, {name: "Provider 1", created_at: now, updated_at: now}) }
  let!(:consumer_version_1) { create(:versions, {number: "1.2.3", order: 1, application_id: consumer_1[:id], created_at: now, updated_at: now}) }
  let!(:consumer_version_2) { create(:versions, {number: "4.5.6", order: 2, application_id: consumer_1[:id], created_at: now, updated_at: now}) }
  ...
  let!(:consumer_2) { create(:applications, {name: "Consumer 2", created_at: now, updated_at: now}) }
  let!(:provider_2) { create(:applications, {name: "Provider 2", created_at: now, updated_at: now}) }
  let!(:consumer_version_3) { create(:versions, {number: "7.8.9", order: 1, application_id: consumer_2[:id], created_at: now, updated_at: now}) }
