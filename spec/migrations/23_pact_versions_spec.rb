...
describe "migrate to pact versions (migrate 22-31)", migration: true do
  ...
  let!(:consumer_version_1) { create(:versions, {number: "1.2.3", order: 1, application_id: consumer[:id], created_at: now, updated_at: now}) }
  let!(:consumer_version_2) { create(:versions, {number: "4.5.6", order: 2, application_id: consumer[:id], created_at: now, updated_at: now}) }
