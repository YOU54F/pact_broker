...
APPLICATION_TESTED_DOCUMENTATION_PATHS = []
APPLICATIONS_NO_DOCUMENTATION = %w[
]
APPLICATION_ROUTES_REQURING_A_DOCUMENTATION_TEST = PactBroker.routes
      .select { | route | route.path_include?("application") }
      .reject { | route | APPLICATIONS_NO_DOCUMENTATION.include?(route.path) }
...
RSpec.describe "application routes" do
  ...
  before do
    ...
    APPLICATION_TESTED_DOCUMENTATION_PATHS << path_template
  end

  after(:all) do
    missed_routes = APPLICATION_ROUTES_REQURING_A_DOCUMENTATION_TEST.reject { | route | APPLICATION_TESTED_DOCUMENTATION_PATHS.include?(route.path) }

    if missed_routes.any? && ENV["DEBUG"] != "true"
      #puts "APPLICATION ROUTES MISSING DOCUMENTATION COVERAGE:"
  ...
  let(:fixture) { expected_interaction(subject, APPLICATION_TESTED_DOCUMENTATION_PATHS.size, comments) }
  ...
  describe "Application" do
    let(:fixture) { remove_deprecated_keys(expected_interaction(subject, APPLICATION_TESTED_DOCUMENTATION_PATHS.size, comments)) }

    let(:path_template) { "/applications/:application_name" }
    ...
    describe "PUT" do
      ...
      let(:http_params) { application_hash.to_json }
      include_examples "supports PUT"
    end

    describe "PATCH" do
      let(:comments) { "PATCH with the Content-Type application/merge-patch+json merges the application's existing properties with those from the request body. Embedded properties (eg. versions) will not be affected." }
