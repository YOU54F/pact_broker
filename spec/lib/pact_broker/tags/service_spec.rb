    ...
    describe Service do
      ...
      describe ".create" do
        ...
        it "creates the new tag" do
          ...
          expect(subject.version.application.name).to eq application_name
        end
        ...
        it "calls the application_service.maybe_set_main_branch" do
          expect(application_service).to receive(:maybe_set_main_branch).with(instance_of(PactBroker::Domain::Application), tag_name)
          subject
        end
      end
      ...
      describe "delete" do
        ...
        let(:second_application_name) { "second_test_application" }
        let(:second_version_number) { "4.5.6" }
        let(:second_options_same_tag_name) { { application_name: second_application_name, application_version_number: second_version_number, tag_name: tag_name }}
