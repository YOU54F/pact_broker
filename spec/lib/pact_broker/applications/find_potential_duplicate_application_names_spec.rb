...
module PactBroker
  module Applications
    describe FindPotentialDuplicateApplicationNames do
      ...
      describe "split" do
        ...
        SPLIT_TEST_CASES.each do | input, output |
          it "splits #{input} into #{output.inspect}" do
            expect(FindPotentialDuplicateApplicationNames.split(input)).to eq output
          end
        end
      end

      describe ".call" do
        subject { FindPotentialDuplicateApplicationNames.call(new_name, existing_names) }
