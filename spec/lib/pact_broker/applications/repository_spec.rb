  ...
  module Applications
    describe Repository do
      describe "#create" do
        ...
        context "when the application does not already exist" do
          before do
            td.create_application("Bar")
            allow_any_instance_of(PactBroker::Domain::Application).to receive(:generate_display_name).and_return("display_name")
          end
          ...
          it "returns the new application" do
            expect(subject).to be_a(PactBroker::Domain::Application)
        ...
        context "when a race condition occurs and the application was already created by another request" do
          before do
            td.create_application("Foo", repository_url: "original")
          end
      ...
      describe "replace" do
        before do
          td.create_application("Bar", main_branch: "foo", repository_namespace: "foo")
          allow_any_instance_of(PactBroker::Domain::Application).to receive(:generate_display_name).and_return("display_name")
        end
        ...
        it "replaces the application" do
      ...
      describe "#find" do
        ...
        it "returns the applications with the given label" do
        ...
        context "when scope applied" do
          it "returns the applications if scope allows" do 
            allow_any_instance_of(Repository).to receive(:scope_for).and_return(PactBroker::Domain::Application) # default, with no scope applied
        ...
        context "when the name is a different case" do
          ...
          context "with case sensitivity turned off and multiple records found", skip: PactBroker::TestDatabase.mysql?  do
            ...
            it "raises an error" do
              expect { subject }.to raise_error PactBroker::Error, /Found multiple applications.*foo-bar/
            end
          end
      ...
      describe "#application_names" do
        before do
          td
            .create_application("Plants")
            .create_application("Animals")
        end

        subject { Repository.new.application_names }

        it "returns an array of application names" do
