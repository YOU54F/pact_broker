require "pact_broker/api/resources/application"
...
module PactBroker::Api
  module Resources
    describe Application do
      describe "PUT" do
        before do
          allow(PactBroker::Applications::Service).to receive(:find_application_by_name).and_return(pacticpant)
        end

        let(:pacticpant) { double("application") }
        let(:path) { "/applications/foo" }
        ...
        context "with a body" do
          ...
          it "creates a new application when the application does not exist" do
            allow(PactBroker::Applications::Service).to receive(:find_application_by_name).and_return(nil)
            subject
          ...
          context "when application exist" do
            ...
            it "updates the existing application" do
              subject
      ...
      describe "PATCH" do
        ...
        context "with a body" do
          ...
          it "creates a new application when the application does not exist" do
            expect(PactBroker::Applications::Service).to receive(:create)
              .with({ name: "foo" })
            subject
          end
      ...
      describe "GET" do
        let(:application) { td.create_consumer("Consumer").and_return(:application) }
        ...
        let(:path) { "/applications/Consumer" }
      ...
      describe "DELETE" do
        before do
          allow(PactBroker::Applications::Service).to receive(:delete)
          allow(PactBroker::Applications::Service).to receive(:find_application_by_name).and_return(pacticpant)
        end

        let(:pacticpant) { double("pacticpant") }

        subject { delete("/applications/Some%20Service" ) }

        context "when the resource exists" do
          it "deletes the pacticpant by name" do
            expect(PactBroker::Applications::Service).to receive(:delete).with("Some Service")
            subject
          end
        ...
        context "when an error occurs" do
          before do
            allow(PactBroker::Applications::Service).to receive(:delete).and_raise("An error")
          end
