describe "Update a application" do
  ...
  context "with a PUT" do
    ...
    context "when the application exists" do
      before do
        td.create_application("Some Consumer")
      end
  ...
  context "with a PATCH" do
    ...
    context "when the application exists" do
      before do
        td.create_application("Some Consumer", repository_name: "existing")
      end
      ...
      it "returns a json body with the updated application" do
      ...
      context "when request body contains embedded labels" do
        context "when labels has values" do
          ...
          it "should not create the labels for the application" do 
            expect(response_body_hash[:_embedded][:labels]).to be_empty
          end

          it "only updates application attribute ignoring the labels" do
