...
module PactBroker
  module Deployments
    class CurrentlyDeployedVersionId < Sequel::Model
      ...
      plugin :upsert, identifying_columns: [:application_id, :environment_id, :target_for_index]
...
#  currently_deployed_version_application_environment_target_index | UNIQUE btree (application_id, environment_id, target_for_index)
# Foreign key constraints:
#  currently_deployed_version_ids_environment_id_fkey | (environment_id) REFERENCES environments(id) ON DELETE CASCADE
#  currently_deployed_version_ids_application_id_fkey | (application_id) REFERENCES applications(id) ON DELETE CASCADE
