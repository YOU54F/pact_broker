...
module PactBroker
  module Deployments
    class ReleasedVersion < Sequel::Model
      ...
      many_to_one :application, :class => "PactBroker::Domain::Application", :key => :application_id, :primary_key => :id
      ...
      dataset_module do
        ...
        def for_application_name(application_name)
          where(application_id: db[:applications].select(:id).where(Sequel.name_like(:name, application_name)))
        end

        def for_application_version_number(application_version_number)
          where(version_id: db[:versions].select(:id).where(Sequel.name_like(:number, application_version_number)))
        end
