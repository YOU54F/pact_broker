...
require "pact_broker/api/contracts/application_create_schema"
  ...
  module Api
    module Resources
      class Applications < BaseResource
        ...
        def from_json
          created_model = application_service.create(parsed_application.to_h)
        ...
        def to_json
          generate_json(application_service.find_all_applications(filter_options, pagination_options, eager_load_associations))
        end

        def generate_json applications
          decorator_class(:deprecated_applications_decorator).new(applications).to_json(**decorator_options)
        end

        def decorator_for model
          decorator_class(:application_decorator).new(model)
        end
        ...
        def schema
          if request.get?
            ...
            PactBroker::Api::Contracts::ApplicationCreateSchema
          end
        end

        def eager_load_associations
          decorator_class(:deprecated_applications_decorator).eager_load_associations
        end
