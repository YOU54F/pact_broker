...
module PactBroker
  module Matrix
    module MatrixRowVerificationDatasetModule
      # Verifications for the provider versions matching the given selectors, where the consumers match the applications in the given selectors.
      # @public
      ...
      def matching_only_selectors_as_provider(resolved_selectors)
        [
          matching_only_selectors_as_provider_where_only_application_name_in_selector(resolved_selectors),
          matching_only_selectors_as_provider_where_not_only_application_name_in_selector(resolved_selectors)
      ...
      # Return verifications where the provider is described by any of the resolved_selectors *that only specify the application NAME*,
      # AND the consumer is described by any of the resolved selectors.
      # If the original selector only specified the application name, we don't need to join to the versions table to identify the required verifications.
      ...
      # @return [Sequel::Dataset<Verification>, nil]
      def matching_only_selectors_as_provider_where_only_application_name_in_selector(resolved_selectors)
        ...
        if application_ids_for_application_only_selectors.any?
          select_verification_columns_with_aliases
            .where(provider_id: application_ids_for_application_only_selectors)
            .where(consumer_id: all_application_ids)
        end
      end
      ...
      # @param [Array<PactBroker::Matrix::ResolvedSelector>] resolved_selectors
      # @return [Sequel::Dataset<Verification>, nil]
      def matching_only_selectors_as_provider_where_not_only_application_name_in_selector(resolved_selectors)
      ...
      # Don't think it's worth splitting this into 2 different queries for selectors with only application name/selectors with version properties,
      # as it's unlikely for there ever to be a query through the UI or CLI that results in 1 selector which only has a application name in it.
      # @private
      # @param [Array<PactBroker::Matrix::ResolvedSelector>] resolved_selectors
